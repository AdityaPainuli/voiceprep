...
export const NoteCard: React.FC<NoteCardProps> = ({ title, content, tags, onSave }) => {
        ...
        <div className="bg-[#161b22] rounded-xl border border-gray-800 p-6 shadow-lg mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-4 border-b border-gray-800 pb-3">
                ...
                <div className="flex items-center gap-2">
                    {tags?.map((tag, i) => (
                        <span key={i} className="px-2 py-0.5 bg-gray-800 text-gray-400 text-xs rounded-full border border-gray-700">
                            #{tag}
                        </span>
                    ))}
                    {onSave && !isEditing && (
...
export const CodeCard: React.FC<CodeCardProps> = ({ code, language, explanation, onRun, isRunning, output, isError }) => {
    return (
        <div className="bg-[#161b22] rounded-xl border border-gray-800 shadow-lg mb-6 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="h-[300px] border-b border-gray-800">
                <CodeEditor
                    language={language}
                    value={code}
                    onChange={() => { }} // Read-only for now in the stream, or we could make it editable
                    onSubmit={() => { }}
...
export const SlideCard: React.FC<SlideCardProps> = ({ title, bulletPoints }) => {
    return (
            ...
            <div className="space-y-4">
                {bulletPoints.map((point, i) => (
                    <div key={i} className="flex items-start gap-4 animate-in fade-in slide-in-from-left-4" style={{ animationDelay: `${i * 150}ms` }}>
                        ...
                        <p className="text-xl text-gray-200 leading-relaxed font-light">{point}</p>
                    </div>
                ))}
            </div>
        </div>
