...
const ManimService_1 = require("./services/ManimService");
...
const manimService = new ManimService_1.ManimService();
...
fastify.post('/api/upgrade', async (request, reply) => {
    try {
        ...
        const prisma = new client_1.PrismaClient(); // Should probably inject or reuse, but this is quick
        const user = await prisma.user.update({
            where: { id: decoded.userId },
            data: { plan }
        });
        // Generate new token with updated plan
        const newToken = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, plan: user.plan }, JWT_SECRET, { expiresIn: '7d' });
...
// ... (keep startServer)
const wss = new ws_2.WebSocketServer({ server: fastify.server });
wss.on('connection', async (ws, req) => {
    ...
    const initializeSession = (mode, config) => {
        // ... (keep existing logic)
        const runInit = () => {
    ...
    ws.on('message', (message) => {
        try {
            ...
            else if (data.type === 'audio' && openAIWs.readyState === ws_1.default.OPEN) {
                const audioAppend = {
            ...
            else if (data.type === 'submit_code' && openAIWs.readyState === ws_1.default.OPEN) {
            ...
            else if (data.type === 'run_code' && openAIWs.readyState === ws_1.default.OPEN) {
                const codeExecution = {
                    type: 'conversation.item.create',
                    item: {
                        type: 'message',
            ...
            else if (data.type === 'next_question' && openAIWs.readyState === ws_1.default.OPEN) {
            ...
            else if (data.type === 'text_message' && openAIWs.readyState === ws_1.default.OPEN) {
                const textMessage = {
                    type: 'conversation.item.create',
                    item: {
                        type: 'message',
