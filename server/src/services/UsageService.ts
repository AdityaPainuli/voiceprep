...
export class UsageService {
  async checkLimit(userId: string, type: 'realtimeMinutes' | 'diagrams' | 'videos' | 'tokens', amount: number = 1) {
    ...
    const currentUsage = (user as any)[fieldMap[type]];
  ...
  async incrementUsage(userId: string, type: 'realtimeMinutes' | 'diagrams' | 'videos' | 'tokens', amount: number = 1) {
    const fieldMap: any = {
      realtimeMinutes: 'realtimeMinutesUsed',
      diagrams: 'diagramsUsed',
      videos: 'videosUsed',
  ...
  async trackSessionDuration(userId: string, startTime: number) {
