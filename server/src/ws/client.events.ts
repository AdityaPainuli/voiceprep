...
export function handleClientMessage(raw: string, ws: any, openAI: any) {
    const data = JSON.parse(raw);
  
    switch (data.type) {
      case 'init_session':
        initializeSession(openAI, data.mode, data.config);
        break;
      ...
      case 'next_question':
        openAI.send({
          ...
          item: {
            type: 'message',
            role: 'user',
            content: [{ type: 'input_text', text: `I am ready for the next question. Please provide a new technical interview question` }],
          },
        });
        openAI.send({ type: 'response.create' });
        break;
